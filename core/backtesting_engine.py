"""
Backtesting Engine for Lighter Point Farming Bot - FIXED VERSION
Added proper session cleanup and improved signal acceptance
"""
...
class BacktestingEngine:
    ...
    async def run_backtest(self, start_date: str, end_date: str, timeframe: str = "5m") -> dict:
        """Run comprehensive backtest with proper cleanup"""
        try:
    ...
    def _should_take_trade(self, signal: dict) -> bool:
        """FIXED: Accept more signal types"""
        try:
            quality = signal.get('quality', 'NEUTRAL')
            
            # FIXED: Accept GOOD, GEM, and some NEUTRAL signals
            if quality in ['GEM', 'GOOD']:
                return True
            elif quality == 'NEUTRAL':
                # Accept NEUTRAL signals with decent scores
                score = signal.get('quality_score', 0)
                return score >= 30  # Reduced from 45
            return False
    ...
    async def _backtest_pair(self, strategy, pair: str, start_date: str, end_date: str, timeframe: str):
    ...
    async def _process_signals(self, strategy, pair: str, data: pd.DataFrame):
        ...
        try:
            ...
            for idx, row in data.iterrows():
                try:
                    ...
                    for trade_id, trade in open_trades.items():
                        exit_result = self._check_trade_exit(strategy, trade, row, current_time)
                        if exit_result:
            ...
            logger.info(f"📊 Processed {signal_count} signals for {pair}")
    ...
    def _check_trade_exit(self, strategy, trade: dict, row: pd.Series, current_time: datetime) -> Optional[dict]:
    ...
    async def _open_trade(self, signal: dict, entry_time: datetime) -> Optional[dict]:
        ...
        try:
            ...
            logger.debug(f"📈 Opened {trade['side']} trade for {trade['pair']} at ${trade['entry_price']:.6f} ({trade['quality']})")
    ...
    async def _close_trade(self, trade: dict, exit_result: dict, exit_time: datetime):
        ...
        try:
            ...
            logger.debug(f"📉 Closed {trade['side']} trade for {trade['pair']}: "
                        f"{trade['pnl_percent']:+.2f}% (${trade['pnl_amount']:+.2f}) - {exit_reason}")
